...
const UserEventCard: React.FC<UserEventCardProps> = ({ image, title, location, date, timeFrom, timeTo, variant, onEdit }) => {
  return (
    <div className="m-8 flex h-44 w-full flex-col rounded-lg border border-gray-200 bg-white shadow hover:bg-gray-100 dark:border-gray-700 dark:bg-gray-800 dark:hover:bg-gray-700 md:flex-row">
      {/* Left: Image */}
      <img src={image} alt={title} className="mx-5 my-auto h-full w-full rounded-lg object-cover md:h-32 md:w-80 " />
      ...
      <div className="flex w-full flex-col justify-between py-6 leading-none">
        ...
        {/* Location */}
        <p className="mb-1 text-sm font-medium text-gray-700 dark:text-gray-400">Location: {location}</p>

        {/* Date */}
        <p className="mb-1 text-sm font-medium text-gray-700 dark:text-gray-400">Date: {date}</p>
